...
import { getHistoryById } from "./content"

type HistoryDetailPageProps = { topicId: string }

export default function HistoryDetailPage({ topicId }: HistoryDetailPageProps) {
    const topic = useMemo(() => getHistoryById(topicId), [topicId])
    ...
    const learningPoints = useMemo(
        () =>
            topic
                ? Array.from(new Set(topic.sections.flatMap((s) => s.keyPoints))).slice(0, 6)
                : [],
        [topic]
    )
    ...
    if (!topic) {
        return (
            <div className="container px-4 py-16 text-center">
                ...
                <Button variant="ghost-emerald" className="mt-4" href="/history">
                    Back to history
                </Button>
            </div>
        )
    ...
    const categoryLabel = topic.category.replace("-", " ")
